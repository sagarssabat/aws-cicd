...
import { getFanMomentPollDetails, sumbitFanPollDetails } from '../../services/fanzone.service'
import {
  getCookieJSON,
  getQueryStringValue,
  isNull,
  ...
  return new Vue({
    ...
    methods: {
      ...
      getUserPollDetails: async function () {
        ...
        let userPollDetails: FanZoneApiResponse = await getFanMomentPollDetails(queryParams)
      ...
      setPollWinnerData() {
        ...
        this.pollWinnerOrder = (max.optionParsed!.order).toString()
        this.pollWinnerTitle = max.optionParsed!.title;
        this.pollWinnerPercentage = this.calcPercentage(max.response_count, pollData.total_response);
        this.pollWinnerImgUrl = max.optionParsed!.imgUrl;
        this.pollWinnerVideoId = max.optionParsed!.videoId;
      },
      setViewData() {
      ...
      appendAutoplay(videoId: string) {
        this.isAutoPlay = "&autoplay=1"
      }
    },
    mounted() {
      this.setUserGuid()
    ...
    computed: {
      ...
      sortPollOptionByOrder: function () {
        let sortedArray: PollOption[] = [];
        let pollData: PollData = this.pollData!
        if (!isNull(this.pollData) && !isNull(pollData.poll_option)) {
          sortedArray = [...pollData.poll_option].sort(function (prev, current) {
            return (prev.optionParsed!.order < current.optionParsed!.order) ? -1 : 1
          })
        }
        return sortedArray;
      }
